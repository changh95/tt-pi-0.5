...
Fused graph (``TT_FUSED=1``): the suffix is carried at 64 rows (action_horizon 50 zero-padded to
the tile boundary; the host keeps rows [:50]), ``embed_actions_fused`` emits the hidden stream in
DRAM (the buffer type of the fused residual op's two addcmul inputs), and ``euler_step_fused`` folds
``velocity = h @ W_out + b; x = x_t + dt * velocity`` into ONE ``dit_minimal_matmul_addcmul_fused``
(``x_t + dt * (h @ W_out + b) * ones``; dt is the op's fp32 scalar attribute exactly as the legacy
``ttnn.mul(velocity, dt)``); needs W_out in bf16 (x_t is bf16: residual format == weight format).
"""
...
class SuffixEmbeddingTTNN:
    ...
    def __init__(
        ...
        Args:
            ...
            device: TTNN device
            fused_cfg: TT_FUSED knobs (None / disabled -> legacy methods only)
        """
        self.config = config
        self.device = device
        self.weights = weights
        self.fused_cfg = fused_cfg
        self._ones_action = None
    ...
    def embed_actions_fused(self, noisy_actions: ttnn.Tensor) -> ttnn.Tensor:
        ...
        legacy action_in_proj linear; DRAM so the first expert layer's fused residual sees its residual
        and gate in the same buffer type (the legacy path's combination for the fused op)."""
        return ttnn.linear(
            noisy_actions,
            self.weights["action_in_proj.weight"],
            bias=self.weights["action_in_proj.bias"],
...
def convert_suffix_weights_to_ttnn(
    ...
    Args:
        ...
        weight_dtype_overrides: per-key weight dtype (fused graph: ``action_out_proj.weight`` in bf16
            because the Euler fused op needs weight format == x_t format); None = legacy dtypes
