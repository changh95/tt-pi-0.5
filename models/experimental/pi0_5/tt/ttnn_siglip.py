...
from models.experimental.pi0_5.common.fused_host import positional_table
from models.experimental.pi0_5.tt.ttnn_gemma import mcast2d_program_config, with_fp32_acc
...
class SigLIPAttentionTTNN:
    ...
    def __init__(
        ...
        self._sdpa_cfg_fused_cache: Dict[int, ttnn.SDPAProgramConfig] = {}
        self._pc_cache: Dict[int, tuple] = {}  # PI05_SIGLIP_PC=mcast2d linear program configs per row count
        ...
        if seq_len not in self._sdpa_cfg_fused_cache:
            ...
            if chunks is not None:
                cfg = ttnn.SDPAProgramConfig(
                    ...
                    q_chunk_size=min(chunks[0], seq_len),
                    k_chunk_size=min(chunks[1], seq_len),
                    exp_approx_mode=False,
                )
            else:
                cfg = ttnn.SDPAProgramConfig(
                    compute_with_storage_grid_size=(min(8, device_grid.x), min(8, device_grid.y)),
                    q_chunk_size=min(256, seq_len),
                    k_chunk_size=min(256, seq_len),
                    exp_approx_mode=False,
                )
            self._sdpa_cfg_fused_cache[seq_len] = cfg
    ...
    def forward_fused(self, hidden_states: ttnn.Tensor) -> ttnn.Tensor:
        ...
        attn_output = ttnn.transformer.scaled_dot_product_attention(
            ...
            is_causal=False,
            scale=self.scale,
            program_config=self._sdpa_config_fused(seq_len),
        )
        ttnn.deallocate(q_heads)
        ttnn.deallocate(k_heads)
        ttnn.deallocate(v_heads)
    ...
    def _program_configs(self, rows: int):
        ...
        if rows not in self._pc_cache:
            self._pc_cache[rows] = (
                mcast2d_program_config(self.device, rows, int(self.wqkv.shape[-2]), int(self.wqkv.shape[-1])),
                mcast2d_program_config(self.device, rows, int(self.wo.shape[-2]), int(self.wo.shape[-1])),
            )
...
class SigLIPMLPTTNN:
    ...
    def forward_fused(self, hidden_states: ttnn.Tensor) -> ttnn.Tensor:
        ...
        x = ttnn.linear(
            ...
            dtype=ttnn.bfloat16,
            memory_config=ttnn.L1_MEMORY_CONFIG,
            activation="gelu" if pc1 is None else None,
            compute_kernel_config=ckc,
            program_config=pc1,
        )
        output = ttnn.linear(
...
class SigLIPBlockTTNN:
    ...
    def forward_fused(self, hidden_states: ttnn.Tensor) -> ttnn.Tensor:
        ...
        attn_output = self.attention.forward_fused(normed)
        ...
        mlp_output = self.mlp.forward_fused(normed)
        ttnn.deallocate(normed)
...
class SigLIPVisionTowerTTNN:
    ...
    def __init__(
        ...
        for i in range(config.num_hidden_layers):
            ...
            self.blocks.append(SigLIPBlockTTNN(config, block_weights, device, fused_cfg=fused_cfg))
    ...
    def _pos_table(self, batch: int) -> ttnn.Tensor:
        ...
        if batch not in self._pos_table_cache:
            ...
            self._pos_table_cache[batch] = ttnn.from_torch(
                positional_table(self._pos_emb_torch, batch),
                dtype=ttnn.bfloat16,
                layout=ttnn.TILE_LAYOUT,
                device=self.device,
                memory_config=ttnn.DRAM_MEMORY_CONFIG,
            )
        return self._pos_table_cache[batch]

    def forward_fused(self, im2col_rm: ttnn.Tensor) -> ttnn.Tensor:
        """``[B, num_patches, 608]`` bf16 ROW_MAJOR host im2col (persistent trace input) ->
        ``[B, num_patches, hidden]`` bf16 (L1). One tilize, the patch linear, the pos-table add, the
        blocks (batched over the B cameras) and the post layer norm."""
        patches = ttnn.tilize(im2col_rm, memory_config=ttnn.L1_MEMORY_CONFIG, use_multicore=True)
        ...
        for block in self.blocks:
            hidden_states = block.forward_fused(hidden_states)
